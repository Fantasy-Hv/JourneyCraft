...
| opening_hours    | JSON     | -    | 营业时间                  | 见下方                                      |
| images           | JSON     | -    | 图片 URL 数组             | ["url1","url2"]                         |
| tags             | JSON     | -    | 美食推荐匹配标签（风味/人群/场景）      | ["老字号","排队","适合聚餐"]                    |
| contact_info     | VARCHAR  | 100  | 联系电话                  | 010-12345678                            |
| rating           | DECIMAL  | 3,2  | 评分                    | 4.8                                     |
| review_count     | INT      | 11   | 评价数                   | 1200                                    |
| heat_score       | INT      | 11   | 热度分                   | 8500                                    |
...
- `scenic_area_id`、`building_id`、`node_id` 用于景区、建筑、导航和推荐联动。
- `node_id` 当前仅保留字段和索引，后续由导航建表脚本补外键到 `t_navigation_road_node.id`。
- `source_facility_id` 只用于历史迁移和兼容，不作为主业务关联。
- `tags` 用于推荐模块匹配用户兴趣、场景和口味。
