...
**preferences 字段结构**:(可后续灵活扩展)

```json
{
  "interests": ["history", "nature", "photo"],  // 兴趣标签
  "transport_type": "walk",                      // 默认出行方式
...
| ------------- | -------- | ---- | -------------- | --------------- |
| id            | BIGINT   | 20   | 主键             | 1               |
| node_id       | BIGINT   | 20   | 关联主导航节点 ID   | 1001            |
...
| tags          | JSON     | -    | 推荐标签列表         | ["历史","热门"]    |
| contact_phone | VARCHAR  | 100  | 联系电话           | 010-12345678    |
...
| node_id        | BIGINT   | 20   | 关联主导航节点 ID                        | 2001       |
...
| tags           | JSON     | -    | 风格/内容元素标签（推荐匹配用）         | ["历史","古风","拍照"] |
...
| node_id        | BIGINT   | 20   | 关联导航节点 ID      | 3001                                                                       |
| name           | VARCHAR  | 100  | 名称             | 游客中心                                                                       |
| type           | INT      | 8    | 设施类型           | **0:卫生间 1:停车场 2:售票处 3:游客中心 4:医疗点 5:ATM 6:自动贩卖机 7:摆渡车站 8:自行车租赁** |
| subtype        | VARCHAR  | 50   | 子类型            | 充电桩/行李寄存/储物柜/母婴室/饮水机                                                   |
| facility_class | TINYINT  | 1    | 设施扩展分类（保留字段）   | 预留                                                                            |
| latitude       | DECIMAL  | 10,8 | 纬度             | 39.917000                                                                  |
| longitude      | DECIMAL  | 11,8 | 经度             | 116.397000                                                                 |
| floor_number   | INT      | 默认   | 所在楼层           | 1                                                                          |
| description    | TEXT     | -    | 描述             | 游客服务中心...                                                                   |
| rating         | DECIMAL  | 3,2  | 评分             | 4.8                                                                        |
| review_count   | INT      | 11   | 评价数            | 1200                                                                       |
| heat_score     | INT      | 11   | 热度分            | 8500                                                                       |
| price_range    | VARCHAR  | 20   | 价格区间           | "免费"                                                                       |
...
| scenic_area_id   | BIGINT   | 20   | 所属景区 ID               | 1                                       |
| building_id      | BIGINT   | 20   | 所属建筑 ID (可为空)         | 1                                       |
| node_id          | BIGINT   | 20   | 预留导航节点字段               | 3001                                    |
| source_facility_id | BIGINT | 20   | 来源设施 ID（兼容迁移）          | 88                                      |
| name             | VARCHAR  | 100  | 名称                    | 全聚德烤鸭店                                  |
| category         | VARCHAR  | 50   | 餐饮类型                  | 餐厅/小吃/食堂/饮品/美食广场/外卖窗口                  |
| cuisine_type     | VARCHAR  | 50   | 菜系/风味                 | 京菜/川菜/粤菜/甜品                             |
| price_level      | TINYINT  | 1    | 价格等级                  | 3                                       |
| avg_price        | DECIMAL  | 10,2 | 人均价格                  | 98.00                                   |
...
| opening_hours    | JSON     | -    | 营业时间                  | 见下方                                      |
| images           | JSON     | -    | 图片 URL 数组             | ["url1","url2"]                         |
| tags             | JSON     | -    | 推荐匹配标签（风味/人群/场景）      | ["老字号","排队","适合聚餐"]                    |
| contact_info     | VARCHAR  | 100  | 联系电话                  | 010-12345678                            |
| rating           | DECIMAL  | 3,2  | 评分                    | 4.8                                     |
| review_count     | INT      | 11   | 评价数                   | 1200                                    |
| heat_score       | INT      | 11   | 热度分                   | 8500                                    |
| recommend_score  | INT      | 11   | 推荐分                   | 7800                                    |
| status           | TINYINT  | 1    | 状态 (1:营业 0:关闭)       | 1                                       |
| created_at       | DATETIME | -    | 创建时间                  | -                                       |
| updated_at       | DATETIME | -    | 更新时间                  | -                                       |
...
- `FoodPlace` 是当前落地的美食主表，不再依赖 `Facility` 承载餐饮业务。
- `scenic_area_id`、`building_id` 用于景区和建筑归属；`node_id` 当前为预留导航节点字段，本阶段不接入美食导航。
- `source_facility_id` 只用于历史迁移和兼容，不作为主业务关联。
- `tags` 用于推荐模块匹配用户兴趣、场景和口味，推荐模块可直接结合 `cuisine_type`、`price_level`、`heat_score` 等字段消费 `FoodPlace`。
...
| ref_type       | TINYINT  | 1    | 引用类型    | 0:无 1:建筑 2:设施 3:拍照点 4:美食             |
...
> zhe / liu 合并后，最终原始拥挤度记录统一由 `scenic` 模块维护，落库表为 `t_crowd_level`。
...
| crowd_count      | INT        | 11    | 人数，可空    | 150                           |
| capacity         | INT        | 11    | 容量，可空    | 500                           |
| predicted_at     | DATETIME   | -     | 预测时间，可空 | 2024-03-25 11:00:00           |
| recorded_at      | DATETIME   | -     | 记录时间      | 2024-03-25 10:00:00           |
| source           | TINYINT    | 1     | 数据来源      | **1:用户上报 2:系统采集**      |
| is_deleted       | TINYINT    | 1     | 逻辑删除      | 0                             |
| created_at       | DATETIME   | -     | 创建时间      | -                             |
| updated_at       | DATETIME   | -     | 更新时间      | -                             |
...
| id            | BIGINT   | 20  | 主键      | 1                                 |
| user_id       | BIGINT   | 20  | 用户 ID   | 1001                              |
| favorite_type | TINYINT  | 1   | 收藏类型    | **0:景点 1:校园 2:建筑 3:设施 4:日记 5:路线 6:美食(预留)** |
...
CREATE INDEX idx_node ON t_building(node_id);
...
-- 美食点表
CREATE INDEX idx_food_scenic ON t_food_place(scenic_area_id);
CREATE INDEX idx_food_building ON t_food_place(building_id);
CREATE INDEX idx_food_node ON t_food_place(node_id);
CREATE INDEX idx_food_category ON t_food_place(category);
CREATE INDEX idx_food_cuisine_type ON t_food_place(cuisine_type);
CREATE INDEX idx_food_recommend_score ON t_food_place(recommend_score DESC);
...
根据课程要求 (200 景区/校园、每景区 20 建筑、通用设施与美食点并存、200 边、10 用户):
...
| end_node_ids    | JSON     | -    | 终点列表 (多目标) | [20, 30, 40]        |
| path_nodes      | JSON     | -    | 路径节点序列     | 见下方                 |
| total_distance  | DECIMAL  | 10,2 | 总距离 (米)    | 3500.50             |
| estimated_time  | INT      | 11   | 预计时间 (秒)   | 4200                |
| transport_modes | JSON     | -    | 交通方式组合     | ["walk", "shuttle"] |
| strategy        | VARCHAR  | 50   | 规划策略       | shortest_time       |
| created_at      | DATETIME | -    | 创建时间       | -                   |
...
**transport_modes 字段结构**:

```json
[
  {"segment": 0, "mode": "walk", "distance": 500, "duration": 400},
  {"segment": 1, "mode": "shuttle", "distance": 1000, "duration": 180}
]
```
...
| path_nodes     | JSON     | -   | 路径节点序列   | [10, 15, 18, 20]    |
| transport_mode | TINYINT  | 1   | 交通方式     | 1:步行 2:自行车 3:电瓶车    |
...
```mermaid
erDiagram
    USER ||--o{ USER_PREFS : "has"
    USER ||--o{ FAVORITE : "favorites"
    USER ||--o{ VIEW_HISTORY : "views"
    USER ||--o{ SEARCH_HISTORY : "searches"
    USER ||--o{ NAVIGATION_HISTORY : "navigates"
    USER ||--o{ NAVIGATION_ROUTE : "creates"
    USER ||--o{ DIARY : "publishes"
    USER ||--o{ EXPENSE_RECORD : "spends"
    USER ||--o{ TRAVEL_GROUP : "creates"
    USER ||--o{ GROUP_MEMBER : "joins"
    USER ||--o{ FACILITY_REVIEW : "reviews"
    USER ||--o{ TRIP : "plans"

    SCENIC_AREA ||--o{ BUILDING : "contains"
    SCENIC_AREA ||--o{ FACILITY : "has"
    SCENIC_AREA ||--o{ ROAD_NODE : "has"
    SCENIC_AREA ||--o{ PHOTO_SPOT : "has"
    SCENIC_AREA ||--o{ CROWD_LEVEL : "records"
    SCENIC_AREA ||--o{ TRIP : "destinations"
    SCENIC_AREA ||--o{ NAVIGATION_ROUTE : "contains"
    SCENIC_AREA ||--o{ NAVIGATION_HISTORY : "records"

    BUILDING ||--o{ FACILITY : "contains"
    BUILDING ||--o{ FLOOR : "has"

    FACILITY ||--o{ FOOD_ITEM : "serves"
    FACILITY ||--o{ FOOD_STALL : "contains"
    FACILITY ||--o{ FACILITY_REVIEW : "receives"

    CUISINE_TYPE ||--o{ FACILITY : "classifies"

    ROAD_NODE ||--o{ ROAD_EDGE : "connects"
    ROAD_NODE ||--o{ CROWD_LEVEL : "measures"
    ROAD_NODE ||--o{ PHOTO_SPOT : "located_at"

    TRAVEL_GROUP ||--o{ GROUP_MEMBER : "includes"
    TRAVEL_GROUP ||--o{ GROUP_PLAN : "generates"
    TRAVEL_GROUP ||--o{ EXPENSE_RECORD : "shares"

    TRIP ||--o{ TRIP_ROUTE : "contains"
    TRIP_ROUTE ||--o{ TRIP_ROUTE_NODE : "includes"

    DIARY ||--o{ COMMENT : "receives"
    DIARY ||--o{ DIARY_LIKE : "receives"

    FAVORITE {
        bigint id PK
        bigint user_id FK
        tinyint favorite_type
        bigint favorite_id
        datetime created_at
    }
...
```sql
-- ScenicArea 表
CREATE INDEX idx_type_city ON `scenic_area`(type, city);
CREATE INDEX idx_heat_score ON `scenic_area`(heat_score DESC);
CREATE INDEX idx_rating ON `scenic_area`(rating DESC);
CREATE INDEX idx_location ON `scenic_area`(latitude, longitude);
CREATE FULLTEXT INDEX ft_description ON `scenic_area`(description);
CREATE FULLTEXT INDEX ft_name ON `scenic_area`(name);

-- Building 表
CREATE INDEX idx_scenic_area_id ON `building`(scenic_area_id);
CREATE INDEX idx_type ON `building`(type);
CREATE INDEX idx_name ON `building`(name);

-- Facility 表
CREATE INDEX idx_scenic_area_id ON `facility`(scenic_area_id);
CREATE INDEX idx_type ON `facility`(type);
CREATE INDEX idx_location ON `facility`(latitude, longitude);
CREATE INDEX idx_name ON `facility`(name);
CREATE INDEX idx_building_id ON `facility`(building_id);
```
...
```sql
-- RoadNode 表
CREATE INDEX idx_scenic_area_id ON `road_node`(scenic_area_id);
CREATE INDEX idx_node_type ON `road_node`(node_type);
CREATE INDEX idx_location ON `road_node`(latitude, longitude);

-- RoadEdge 表
CREATE INDEX idx_from_node_id ON `road_edge`(from_node_id);
CREATE INDEX idx_to_node_id ON `road_edge`(to_node_id);
CREATE INDEX idx_scenic_area_id ON `road_edge`(scenic_area_id);

-- NavigationRoute 表
CREATE INDEX idx_user_id ON `navigation_route`(user_id);
CREATE INDEX idx_scenic_area_id ON `navigation_route`(scenic_area_id);
CREATE INDEX idx_created_at ON `navigation_route`(created_at DESC);

-- NavigationHistory 表
CREATE INDEX idx_user_id ON `navigation_history`(user_id);
CREATE INDEX idx_navigated_at ON `navigation_history`(navigated_at DESC);
```
...
```sql
-- Favorite 表
CREATE UNIQUE INDEX uk_user_type_target ON `favorite`(user_id, favorite_type, favorite_id);
CREATE INDEX idx_user_id ON `favorite`(user_id);
CREATE INDEX idx_type_target ON `favorite`(favorite_type, favorite_id);

-- ViewHistory 表
CREATE INDEX idx_user_id ON `view_history`(user_id);
CREATE INDEX idx_viewed_at ON `view_history`(view_time DESC);
CREATE INDEX idx_user_target ON `view_history`(user_id, target_type, target_id);
...
```sql
-- TravelGroup 表
CREATE INDEX idx_creator_id ON `travel_group`(creator_id);
CREATE INDEX idx_status ON `travel_group`(status);
CREATE INDEX idx_trip_date ON `travel_group`(trip_date);

-- GroupMember 表
CREATE UNIQUE INDEX uk_group_user ON `group_member`(group_id, user_id);
CREATE INDEX idx_group_id ON `group_member`(group_id);
CREATE INDEX idx_user_id ON `group_member`(user_id);
```
...
```sql
-- ExpenseRecord 表
CREATE INDEX idx_user_id ON `expense_record`(user_id);
CREATE INDEX idx_group_id ON `expense_record`(group_id);
CREATE INDEX idx_expense_time ON `expense_record`(expense_time DESC);
CREATE INDEX idx_category ON `expense_record`(category);
```
...
db.diary.createIndex({ userId: 1, createdAt: -1 });
db.diary.createIndex({ scenicAreaId: 1, rating: -1 });
db.diary.createIndex({ status: 1, publishedAt: -1 });
db.diary.createIndex({ heatScore: -1 });
db.diary.createIndex({ tags: 1 });
db.diary.createIndex({ title: "text", content: "text" });  // 全文检索
db.diary.createIndex({ tripId: 1 });  // 关联行程查询

// Comment 集合
db.diary_comments.createIndex({ diaryId: 1, createdAt: 1 });
db.diary_comments.createIndex({ userId: 1 });
db.diary_comments.createIndex({ parentId: 1 });  // 回复查询

// 复合索引示例
db.diary.createIndex({ scenicAreaId: 1, status: 1, heatScore: -1 });  // 热门日记
db.diary.createIndex({ userId: 1, status: 1, visitDate: -1 });  // 用户日记列表
```
...
| 源实体         | 关系类型 | 目标实体              | 说明         |
| ----------- | ---- | ----------------- | ---------- |
| User        | 1:1  | UserPreferences   | 用户拥有唯一偏好设置 |
| User        | 1:N  | Favorite          | 用户可收藏多个实体  |
| User        | 1:N  | ViewHistory       | 用户可有多条浏览记录 |
| User        | 1:N  | SearchHistory     | 用户可有多条搜索记录 |
| User        | 1:N  | NavigationHistory | 用户可有多条导航记录 |
| User        | 1:N  | NavigationRoute   | 用户可创建多条路线  |
| User        | 1:N  | Diary             | 用户可发布多篇日记  |
| User        | 1:N  | ExpenseRecord     | 用户可有多笔消费记录 |
| User        | 1:N  | GroupMember       | 用户可加入多个小组  |
| User        | 1:N  | FaciltyReview     | 用户可评价多个设施  |
| ScenicArea  | 1:N  | Building          | 景区包含多个建筑物  |
| ScenicArea  | 1:N  | Facility          | 景区包含多个通用设施 |
| ScenicArea  | 1:N  | FoodPlace         | 景区包含多个美食点   |
| ScenicArea  | 1:N  | RoadNode          | 景区包含多个路网节点 |
| ScenicArea  | 1:N  | PhotoSpot         | 景区包含多个拍照点  |
| ScenicArea  | 1:N  | CrowdLevel        | 景区有多条拥挤度记录 |
| Building    | 1:N  | Facility          | 建筑物包含多个设施  |
| Building    | 1:N  | FoodPlace         | 建筑物可包含多个美食点 |
| Building    | 1:N  | Floor             | 建筑物包含多个楼层  |
| Facility    | 1:N  | FacilityReview    | 设施可有多条评价   |
| FoodPlace   | 1:N  | FoodItem          | 美食点可扩展多个菜品  |
| FoodPlace   | 1:N  | FoodStall         | 美食点可扩展多个摊位  |
| RoadNode    | 1:N  | RoadEdge          | 节点可连接多条边   |
| RoadNode    | 1:N  | PhotoSpot         | 节点可设置为拍照点  |
| TravelGroup | 1:N  | GroupMember       | 小组包含多个成员   |
| TravelGroup | 1:N  | GroupPlan         | 小组可生成多个方案  |
| Trip        | 1:N  | TripRoute         | 行程包含多条路线   |
| TripRoute   | 1:N  | TripRouteNode     | 路线包含多个节点   |
| Diary       | 1:N  | Comment           | 日记可有多条评论   |

## 14.2 多态关联说明

### Favorite 表的多态关联

| favorite_type | 目标表              | target_id 关联字段      |
| ------------- | ---------------- | ------------------- |
| 0             | scenic_area      | scenic_area.id      |
| 1             | scenic_area (校园) | scenic_area.id      |
| 2             | building         | building.id         |
| 3             | facility         | facility.id         |
| 4             | diary (MongoDB)  | diary._id           |
| 5             | navigation_route | navigation_route.id |
| 6             | food_place       | food_place.id       |

### ViewHistory 表的多态关联

| target_type | 目标表              | target_id 关联字段 |
| ----------- | ---------------- | -------------- |
| 0           | scenic_area      | scenic_area.id |
| 1           | scenic_area (校园) | scenic_area.id |
| 2           | diary (MongoDB)  | diary._id      |
| 3           | facility         | facility.id    |
